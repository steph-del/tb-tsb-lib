...
| ---                            | ---     | ---                              | ---                         | ---                         | ---         |
| level                          | non     | `string`                         | `idiotaxon`, `synusy`, `microcenosis`, etc. | `idiotaxon` | le niveau d'intégration. Pour le Cel, ce sera toujours 'idiotaxon' |
| tbRepositoriesConfig           | non     | `Array<TbRepositoryConfigModel>` | tableau d'objets            | -                           | Liste des référentiels provenant de l'API de Tela Botanica à ajouter dynamiquement |
| defaultRepository              | non     | `string`                         | un nom de référentiel       | Le premier accessible       | référentiel à utiliser par défaut |
| fixedRepository                | non     | `string`                         | un nom de référentiel       | -                           | forcer l'utilisation d'un référentiel |
| allowEmptyRepository           | non     | `boolean`                        | `true`, `false`             | `true`                      | autorise la saisie d'une donnée hors référentiel (ajoute un référentiel 'Autre/inconnu' à la liste des ref.) |
| allowFreeValueIfNoResults      | non     | `boolean`                        | `true`, `false`             | `true`                      | autorise la saisie d'une valeur libre s'il n'y a aucun résultat avec l'auto-completion |
| autoSelectValueIfOnlyOneResult | non     | `boolean`                        | `true`, `false`             | `false`                     | si la recherche ne renvoie qu'un seul résultat, il est automatiquement selectionné |
| startSearchAtEdit              | non     | `boolean`                        | `true`, `false`             | `false`                     | lance automatiquement une recherche quand le module pass en mode 'édition' |
| autoComplete                   | non     | `boolean`                        | `true`, `false`             | `true`                      | si `false`, pas d'autocomplétion, le module renvoie tous les résultats |
| autoResetWhenSelected          | non     | `boolean`                        | `true`, `false`             | `true`                      | remet l'input de saisie à zéro après la saisie |
| showRepositoryInput            | non     | `boolean`                        | `true`, `false`             | `true`                      | affiche le sélecteur de référentiels |
| inputFullWidth                 | non     | `boolean`                        | `true`, `false`             | `true`                      | `width="100%"` |
| floatLabel                     | non     | `string`                         | `auto`, `always`, `never`   | `auto`                      | c'est un paramètre de l'input Material |
| hintRepoLabel                  | non     | `boolean`                        | `true`, `false`             | `true`                      | c'est un paramètre de l'input Material |
| placeholder                    | non     | `string`                         | une chaîne de caractères    | -                           | pour modifier le placeholder par défaut |
| editingPlaceholder             | non     | `string`                         | une chaîne de caractères    | `Modifier une donnée`       | placeholder lors de l'édition d'une donnée |
| showAuthor                     | non     | `boolean`                        | `true`, `false`             | `true`                      | affiche les autorités dans l'autocomplete |
| showRepositoryDescription      | non     | `boolean`                        | `true`, `false`             | `false`                     | affiche la description du référentiel |
| attachRawData                  | non     | `boolean`                        | `true`, `false`             | `false`                     | ajoute l'objet rawData à la réponse |
| emitOccurenceOnBlur            | non     | `boolean`                        | `true`, `false`             | `false`                     | emet l'occurence quand l'input perd le focus et uniquement si l'occurence n'a pas été selectionnée dans la liste déroulante des résultats (= n'est pas liée à un référentiel) |
| **updateData**                 | non     | `RepositoryItemModel`            | objet                       | -                           | pour mettre à jour une donnée (bien spécifier l'occurenceId). Le module se met en mode 'isEditing', pré-rempli le champ de recherche, change le référentiel courant sur le référentiel de la donnée à éditer. Un événement updatedData ou cancelUpdateData est forcément renvoyé, selon l'action de l'utilisateur, avant de quitter le mode 'isEditing' |
| startWithValue                 | non     | `RepositoryItemModel`            | objet                       | -                           | valeur par défaut lors de l'initialisation du module. Si le référentiel fournit n'est pas accessible, les champs repository et input ne sont pas mis à jour. Sinon, ils le sont sans déclancher d'évènement. |
| restoreRepositoryValueAfterEditing | non | `boolean`                        | `true`, `false`             | `false`                     | si true, restaure la valeur précédente du référentiel une fois l'édition terminée |
| enabled                        | non     | `boolean`                        | `true`, `false`             | `true`                      | si false, desactive les champs de saisie
| reset                          | non     | `boolean`                        | `true`, `false`             | `false`                     | RAZ du composant si true |
